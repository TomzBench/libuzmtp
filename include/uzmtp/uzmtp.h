...
// Stack allocator for a msg of a specified size. use uzmtp_stack_msg_data to
// populate the data of the message
...
// initialize a message on the stack, where data is located on the heap
#define uzmtp_stack_msg_init_from_data(_name, _f, _d, _l)                      \
    uzmtp_msg_opaque(0) _name;                                                 \
    uzmtp_msg_init((uzmtp_msg_s*)&(_name), (_f), *(_d), (_l), 1)

// initialize a message on the stack where the data is located somewhere else
...
// The rest of the stack macros are just helpers to cast the opaque stack object
// to the pointer type.
